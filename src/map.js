...
const SimpleMap = () => {
  ...
  const readAloud = (instructions) => {
    const maxLength = 1500; // Maximum length of text that can be spoken in one go
    let chunk = '';
    let chunkIndex = 0;
    
    while (chunkIndex < instructions.length) {
      chunk = instructions.slice(chunkIndex, chunkIndex + maxLength).join('. ');
      const utterance = new SpeechSynthesisUtterance(chunk);
      window.speechSynthesis.speak(utterance);
      chunkIndex += maxLength;
    }
  };
